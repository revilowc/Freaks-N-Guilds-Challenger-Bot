...
class BeginView(discord.ui.View):
    ...
    async def button1(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        await interaction.response.send_message("Your eyes flutter open. The room is dark, dimly lit by a yellow glowing orb in the center. You wonder how you got here, but the memory is fuzzy.\n\nYou feel weightless... because you’re floating? Holy ship you’re under water!\n\nYou look down at your hands. You’re a celestial, that’s for sure, sent on a special mission to find... what was it. A rune? There was a lot of chatter in your head about a whitelist thing, is that what you were after? Thank God (err, World Maker) that you’re a Celestial, you don’t need to breathe underwater.\n\nYou swim towards the yellow glowing orb. It’s nothing special, except for the fact that it’s a glowing orb, underwater, with no source of electricity. Damn, high bar to impress these days. You make a mental note of it as you glance around the small room. It’s about 10 meters by 15 meters and there’s an iron door on each wall. The ground is covered in flat gray stones. There is nothing else of note in the room.", view=view, ephemeral=True)
...
You’re glad you can’t smell underwater because this room is as gory as a horror movie. Bones float in the murky water, hauntingly suspended all around the room. You gasp as you notice a Celestial’s skull spinning slowly in one corner.
...
You snag some seaweed to snack on later. Could be tasty. After swimming a lap around the room, you’ve noted 3 iron doors.
                                                """, view=view, ephemeral=True)
...
A mountain of what looks to be sand sits idly in the center of the room. You swim toward it, pushing it around. A few crabs scuttle from beneath the pile, clawing their way away from you. You dig deeper into the sand, hoping to find something useful. Perhaps even some mana? Or maybe some NFTs in the Sandbox? Nope. Nothing. You decide to move on to something more exciting. Something more... freaky.
...
Images showing Ogres line the room, in various poses and holding sharp looking weapons. It seems like some of their history has been immortalized here. Detailed drawings of their greatest loot and most vicious battles can be seen on the opposite side of the room. All this culture, you never knew these freaks had.

You snap back to your task. Your task isn’t learning about Ogres, it’s finding the rune. No time to waste. But will you waste your time going through pointless doors?
""", view=view, ephemeral=True)
...
A huge Ogre snaps it’s gaze toward you, alerted by the sound! It readies it’s club, prepared to swing, but you’re a Celestial. Celestials can handle Ogres with ease. With a flick of your wrist, you send a blast of magical energy shooting through the water, stunning the Ogre. It gurgles a bit and closes its eyes, floating to the bottom, stunned.
...
You recognize this room instantly. The stone floor. The barren room. It’s dark, dimly lit by a yellow glowing orb in the center. You can’t stay here long, you’ve got a quest to do. Just keep swimming, you think, wondering how the hell you are supposed to find this rune in time.
...
Huge rusting pipes line the ceiling and floors. It’s as if you stepped (or swam) into a steampunk movie, they’re everywhere. The water is slightly rust colored and tastes like $FBX. Nice. Too bad you can’t use alchemy to turn it into something valuable.
...
You take a second to carve your initials in the wood, FNG. You gotta leave your mark.
...
An orange orb casts a spooky glow onto the sharp metal objects around this room. As you look closer you realize this room is packed to the brim with weapons including daggers, clubs, maces, swords, axes, and other medieval looking torture devices. Clumps of armor sit sunken on the floor, rusting from the salt water in which they reside.

You think about snagging a dagger, but then you remember that you’re a Celestial and have magic powers. Physical weapons are for those lame freaks.
...
class Dropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.values[0] == "4":
            await interaction.response.send_message("""Using your finger, you traced the number 4 into the empty grid, showing where the orbs resided. The bioluminescent bacteria around you begins to swirl around the podium, slowly at first but getting faster.

You can hear clanging in the background. In a flash, the glowing bioluminescent lights coalesce around the gold orb to form the legendary rune you have been seeking!
...
Congratulations on completing the quest! You will gain a the knight role and whitelisted role for completing this quest. Please go to the enlisted chat and send your wallet address there! Please do not spam that chat otherwise.""", ephemeral=True)
            ...
            await enlistedchannel.send(f"{interaction.user.mention} Congratulations on completing quest 4! Please send your wallet address here and I will store it for you.")
            await interaction.user.remove_roles(peasantrole, reason="Member completed quest 4")
            await interaction.user.add_roles(knightsrole, whitelistrole, reason="Member completed quest 4")
...
class Challenge4(commands.Cog):
    ...
    async def collect(self, ctx):
        ...
        user = self.client.get_user(933156398274068480)
        ...
        for idandwallet in allidsandwallets:
            ...
            if idandwallet.split("/")[0] in completedquest4:
                try:
                    member = self.client.get_user(int(idandwallet.split("/")[0]))
                    completedquest4[completedquest4.index(idandwallet.split(
                        "/")[0])] = member.name + "#" + member.discriminator + "/" + idandwallet.split("/")[1]
                except:
                    continue
        ...
        for id in completedquest4:
            if "/" not in id:
                try:
                    member = self.client.get_user(int(id))
                    completedquest4[completedquest4.index(
                        id)] = member.name + "#" + member.discriminator + "/" + "!!!!!!!NO WALLET ADDRESS SUBMITTED!!!!!!!"
                except:
                    continue
    ...
    async def quest4(self, ctx):
        ...
        await ctx.respond(embed=discord.Embed(description="**Created quest 4 message for users!**", color=embedcolor), ephemeral=True)
